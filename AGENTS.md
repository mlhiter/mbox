...
- Project member records register `user`, `service_account`, or `automation` principals with `owner`, `operator`, or `viewer` roles for a project. They are not trusted identity by themselves. The current shared-token API does not enforce route authorization from these records, and audit actor/source labels are still not trusted identity.
...
- `MBOX_PROJECT_RBAC_ENFORCEMENT_ENABLED` is disabled by default and currently enforces only the starter `sandbox.launch`, `runtime.operate`, `artifact.write`, `policy.manage`, and `credential.manage` actions. When enabled together with trusted principal headers, sandbox creation, template validation launches, active runtime target/log/event/preview/terminal/session/task/workspace-content paths, artifact reference creation, workspace artifact capture, and client artifact-content upload require the caller to match an `owner` or `operator` project member; project launch/quota policy `PUT` routes and project credential-reference create/delete routes require an `owner` project member. Credential list/get routes remain read visibility and do not expose secret values. Do not imply broader route-level RBAC exists yet.
- `GET /v1/projects/{projectID}/authorization?action=...` is project authorization preflight. It maps known actions to required roles and explains the current caller/member state. It reports action-level `enforced`; currently only `sandbox.launch`, `runtime.operate`, `artifact.write`, `policy.manage`, and `credential.manage` can be route-enforced.
