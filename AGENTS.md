...
- The implemented HTTP surface is `GET /healthz`, `GET /v1/info`, `GET /v1/auth/caller`, `GET /v1/runtime/orphans`, CRUD for `/v1/projects`, `/v1/templates`, and `/v1/sandboxes`, project launch policy routes, project authorization preflight, project member role registry routes, project credential-reference routes, project usage summary, sandbox lifecycle routes `POST /v1/sandboxes/{id}/stop` and `/start`, read-only template/sandbox boundary summary routes, plus sandbox runtime target, logs, events, preview ports, preview proxy, terminal, runtime session routes, execution task routes, and artifact reference routes.
...
- Product audit events are best-effort product records. They include successful mutations plus selected `policy.denied` events for launch-policy, active-sandbox quota, retained-artifact-byte quota, and starter RBAC denials. OpenAPI and the TypeScript SDK publish known audit actions and the current `policy.denied` metadata shape for client rendering. Do not describe them as a strong transactional audit log, authentication identity, or a general failure-event stream.
- Project member records register `user`, `service_account`, or `automation` principals with `owner`, `operator`, or `viewer` roles for a project. They are not trusted identity by themselves. The current shared-token API does not enforce route authorization from these records, and audit actor/source labels are still not trusted identity. When project RBAC enforcement and trusted principal headers are both enabled, project creation seeds the trusted caller as an `owner` member for that new project so `member.manage` can bootstrap without adding login or invite flows.
- `GET /v1/auth/caller` is a read-only caller/auth boundary handshake. It reports anonymous local mode, accepted shared-token mode, or explicitly enabled trusted-header mode. Trusted headers are disabled by default through `MBOX_TRUSTED_PRINCIPAL_HEADERS_ENABLED=false` and use `X-Mbox-Principal` plus `X-Mbox-Principal-Type` unless overridden.
- Trusted principal headers are a reverse-proxy/test integration point only. They can mark a caller as `rbacTrusted=true` for caller visibility, authorization preflight matching, and explicitly enabled project RBAC actions; do not use them to imply login, user sessions, audit identity, secret access, or broader runtime permissions.
- `MBOX_PROJECT_RBAC_ENFORCEMENT_ENABLED` is disabled by default and currently enforces only the starter `sandbox.launch`, `runtime.operate`, `artifact.write`, `policy.manage`, `credential.manage`, and `member.manage` actions. When enabled together with trusted principal headers, sandbox creation, template validation launches, active runtime target/log/event/preview/terminal/session/task/workspace-content paths, artifact reference creation, workspace artifact capture, and client artifact-content upload require the caller to match an `owner` or `operator` project member; project launch/quota policy `PUT` routes plus project credential-reference and member create/delete routes require an `owner` project member. Credential and member list/get routes remain read visibility and credential reads do not expose secret values. Do not imply broader route-level RBAC exists yet.
- `GET /v1/projects/{projectID}/authorization?action=...` is project authorization preflight. It maps known actions to required roles and explains the current caller/member state. It reports action-level `enforced`; currently only `sandbox.launch`, `runtime.operate`, `artifact.write`, `policy.manage`, `credential.manage`, and `member.manage` can be route-enforced.
