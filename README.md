...
go run ./cmd/mbox --api-url http://127.0.0.1:18080 runtime orphans --summary-table --resolve-project-names --project-id <project-id>
go run ./cmd/mbox --api-url http://127.0.0.1:18080 templates boundary <template-id> --project-id <project-id> --summary
go run ./cmd/mbox --api-url http://127.0.0.1:18080 sandboxes boundary <sandbox-id> --summary
go run ./cmd/mbox --api-url http://127.0.0.1:18080 projects policy <project-id> --summary
go run ./cmd/mbox --api-url http://127.0.0.1:18080 projects quota-policy <project-id> --summary
```

The resource inventory lists mbox-managed `SandboxClaim` and `SandboxTemplate` objects reported by the runtime auditor, with a live summary by kind, namespace, label-derived owner, label-derived project, and observed workload shape. Use `runtime resources --summary` when a script needs only that filtered JSON summary object, or `runtime resources --summary-table` when an operator wants a human-readable total, kind/namespace/project/owner distribution, and workload rollup without the full resource rows. Add `--resolve-project-names` to runtime resource or orphan summary tables, or to `projects usage <project-id> --summary --runtime-attribution`, when the CLI should make a read-only `/v1/projects` request and render known project IDs as `name (short-id)`; JSON output and runtime API responses stay unchanged. The Web Runtime view also renders `summary.byProject` as clickable project attribution so operators can jump from all resources into the same `projectId` filter. For `SandboxClaim` rows, inventory also includes a best-effort runtime observation: resolved runtime sandbox, selected Pod phase, Pod counts, container readiness, restart count, summed Pod requests/limits, and workspace PVC state when available. The inventory and orphan audit can be scoped by namespace, project owner label, kind, or a combination of those filters. The `--project-id` filter matches mbox-managed runtime resources that carry `mbox.dev/project-id`; it is mainly useful for project-owned `SandboxClaim` rows and should not be read as a full RBAC or quota boundary. `summary.byProject` is computed after the same filters and only counts resources with a non-empty runtime owner project label. The orphan audit compares that inventory with product records and reports resources whose labels no longer line up cleanly, including cleanup-pending soft-deleted sandboxes. Use `runtime orphans --summary-table` when an operator wants the existing read-only orphan audit rendered as reason counts plus resource/project/status/message/evidence rows; default `runtime orphans` output remains JSON for scripts. Template and sandbox boundary commands keep JSON by default and add `--summary` for a human-readable runtime safety view over namespace, ServiceAccount, token automount, launch policy, Secret references, credential references, network/lifecycle projection, runtime reference, and checks. Project usage, member, credential-reference, launch-policy, and quota-policy commands also keep JSON by default and add `--summary` for human-readable operator triage over the same API responses. The workload rollup is not metrics-server utilization, quota, billing, or product-record usage; boundary and policy/quota summaries are not secret-value access, full RBAC, credential mounting, capacity reservation, or live Kubernetes metrics.
...
Use `--request-id <id>` when a script needs to line up a command with API logs and `audit_events.metadata.requestId`. Use `--filter-request-id <id>` on `audit-events` reads to retrieve the matching product audit events, `--operation <operation>` and `--reason <reason>` to narrow typed metadata such as `policy.denied` operation and denial reason values, and RFC3339 `--since` / `--until` windows to inspect a known run interval. Add `--summary` or `--policy-denied-summary` to `audit-events` or `projects audit-events` when an operator wants a grouped, human-readable rollup over the same existing filters; both summary modes include project IDs and request IDs from returned audit rows when present. Add `--resolve-project-names` to either summary mode when the CLI should make a read-only `/v1/projects` request and render known project IDs as `name (short-id)`; JSON audit output and API responses stay unchanged. Request IDs and audit labels are correlation/debugging context, not auth claims or idempotency keys.
