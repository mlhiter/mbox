...
Useful environment variables:
  ...
  MBOX_POSTGRES_PORT
  MBOX_API_TOKEN
  MBOX_TRUSTED_PRINCIPAL_HEADERS_ENABLED
  MBOX_TRUSTED_PRINCIPAL_HEADER
  MBOX_TRUSTED_PRINCIPAL_TYPE_HEADER
  MBOX_PROJECT_RBAC_ENFORCEMENT_ENABLED
  MBOX_ARTIFACT_CONTENT_BACKEND
  MBOX_ARTIFACT_CONTENT_DIR
  MBOX_ARTIFACT_CONTENT_S3_ENDPOINT
  MBOX_ARTIFACT_CONTENT_S3_REGION
  MBOX_ARTIFACT_CONTENT_S3_BUCKET
  MBOX_ARTIFACT_CONTENT_S3_PREFIX
  MBOX_ARTIFACT_CONTENT_S3_ACCESS_KEY_ID
  MBOX_ARTIFACT_CONTENT_S3_SECRET_ACCESS_KEY
  MBOX_ARTIFACT_CONTENT_S3_FORCE_PATH_STYLE
EOF
}
...
start_api() {
	...
	(
		...
		if [[ -n "${MBOX_API_TOKEN:-}" ]]; then
			export MBOX_API_TOKEN
		fi
		for name in \
			MBOX_TRUSTED_PRINCIPAL_HEADERS_ENABLED \
			MBOX_TRUSTED_PRINCIPAL_HEADER \
			MBOX_TRUSTED_PRINCIPAL_TYPE_HEADER \
			MBOX_PROJECT_RBAC_ENFORCEMENT_ENABLED; do
			if [[ -n "${!name:-}" ]]; then
				export "$name"
			fi
		done
		if [[ -n "${MBOX_ARTIFACT_CONTENT_BACKEND:-}" ]]; then
			export MBOX_ARTIFACT_CONTENT_BACKEND
		fi
		if [[ -n "${MBOX_ARTIFACT_CONTENT_DIR:-}" ]]; then
			export MBOX_ARTIFACT_CONTENT_DIR
		fi
		for name in \
			MBOX_ARTIFACT_CONTENT_S3_ENDPOINT \
			MBOX_ARTIFACT_CONTENT_S3_REGION \
			...
			MBOX_ARTIFACT_CONTENT_S3_ACCESS_KEY_ID \
			MBOX_ARTIFACT_CONTENT_S3_SECRET_ACCESS_KEY \
			MBOX_ARTIFACT_CONTENT_S3_FORCE_PATH_STYLE; do
			if [[ -n "${!name:-}" ]]; then
				export "$name"
			fi
		done
