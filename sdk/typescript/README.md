...
if (denied) {
  console.log(denied.metadata.operation, denied.metadata.reason, denied.metadata.matchedMemberRole)
}
...
The command fetches `/v1/openapi.json` from the API base URL and verifies every route-backed SDK helper in `SDK_ROUTE_CONTRACT` has a matching OpenAPI path, method, SDK-used query parameter declarations, route auth metadata, focused request body shape, and focused response shape. It also checks reverse route coverage: ordinary published OpenAPI operations must have SDK route contract entries. The intentional non-helper exceptions are the terminal WebSocket upgrade route and preview proxy pass-through route, which are not ordinary JSON SDK helpers. Auth checks cover the bearer security scheme, explicit public operations, private bearer operations, and `401` responses. Request checks cover JSON schema refs and binary upload media types. Response checks cover direct schema refs, list wrappers with required `items` arrays, list item refs, NDJSON task-event streams, binary responses, and no-content delete routes. It also checks the starter `SDK_SCHEMA_CONTRACT` for required fields, properties, selected forbidden properties, selected nullable properties, selected primitive property types, selected string property formats, selected string-map value types for SDK-consumed `Record<string,string>` fields, selected enum values, and selected array item enum values that the SDK types already rely on, including health and shared error response fields, `APIInfo` runtime/artifact capability and compatibility fields, `CallerInfo` mode/principal-type values, selected boolean boundary/status flags, project authorization action/evaluation values plus required-role/available-action array item values, project policy/quota enforcement values, project quota policy numeric limit fields, nullable PATCH clear fields such as `ProjectUpdate.defaultTemplateId` and `SandboxUpdate.runtimeRef`, project usage count/byte fields, date-time formats for SDK-consumed timestamp fields, UUID formats for selected SDK-consumed product record ID fields, string maps for template env, boundary resource requests, runtime labels, and request/limit maps, SDK-consumed command/startup command arrays as string arrays, project member principal/role values, project credential type values, boundary summary/check/credential-reference values, template validation decision values, runtime inventory summary/workload/observation/storage count fields, runtime log target/log fields, sandbox/session/task status and event values, artifact kind values, artifact and retained-content size fields, retained-content storage provider values, runtime resource owner kind values, runtime orphan reasons, orphan status values, the gated orphan-cleanup confirmation literal, and `PolicyDeniedAuditMetadata` numeric quota hints plus operation, policy kind, enforcement, RBAC, and caller enum values. It can also read a saved OpenAPI JSON file.
