...
export const SDK_SCHEMA_CONTRACT = [
  ...
  },
  {
    ...
    arrayItemTypes: [
      ...
      { property: "allowedSecretRefs", type: "string" },
      { property: "controllerPermissions", type: "string" },
      { property: "runtimeAccess", type: "string" },
      { property: "cleanup", type: "string" },
    ],
  },
  {
    ...
    arrayItemTypes: [{ property: "evidence", type: "string" }],
  },
  {
    schema: "BoundaryPort",
  ...
  },
  {
    ...
    propertyTypes: [
      { property: "total", type: "integer" },
      { property: "retainedContent", type: "integer" },
      { property: "referencedBytes", type: "integer" },
      { property: "retainedBytes", type: "integer" },
      { property: "file", type: "integer" },
      { property: "directory", type: "integer" },
      { property: "log", type: "integer" },
      { property: "report", type: "integer" },
      { property: "screenshot", type: "integer" },
      { property: "image", type: "integer" },
      { property: "link", type: "integer" },
      { property: "other", type: "integer" },
    ],
  },
  {
...
export function checkOpenAPIAlignment(
  ...
  for (const schemaContract of schemas) {
    ...
    for (const itemType of schemaContract.arrayItemTypes ?? []) {
      checkedSchemaArrayItemTypes += 1
      const actualType = schemaArrayItemType(schema, itemType.property)
      if (actualType !== itemType.type) {
        missing.push({
          ...schemaContract,
          reason: "schema-array-item-type-mismatch",
          property: itemType.property,
          expectedType: itemType.type,
          actualType,
        })
      }
    }
...
function schemaArrayItemType(schema: Record<string, unknown>, property: string) {
  if (!isRecord(schema.properties)) {
    return undefined
  }
  const propertySchema = schema.properties[property]
