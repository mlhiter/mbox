...
- Done: CLI audit summary project-name ergonomics add `audit-events --summary --resolve-project-names` and `--policy-denied-summary --resolve-project-names`, resolving known project IDs through `/v1/projects` only for human-readable output while preserving raw JSON, audit API responses, best-effort audit semantics, and non-identity project attribution.
- Done: SDK/OpenAPI policy-denial operation enum alignment guards `PolicyDeniedAuditMetadata.operation` values used by the SDK `PolicyDeniedOperation` union, so generated-client preparation catches operation drift without changing audit behavior, identity trust, or failure-event coverage.
- Done: SDK/OpenAPI policy-denial RBAC/caller enum alignment guards `PolicyDeniedAuditMetadata.authorizationAction`, `callerMode`, `callerPrincipalType`, `principalType`, and `role` values used by SDK starter RBAC metadata types, keeping generated-client preparation aligned without changing audit behavior, identity trust, or route enforcement.
- Done: SDK/OpenAPI policy-denial policy/quota enum alignment guards `PolicyDeniedAuditMetadata.policyKind` (`launch`/`quota`) and `enforcement` (`disabled`/`enforced`) values used by SDK policy-denial metadata types, keeping generated-client preparation aligned without changing audit behavior, quota semantics, identity trust, or route enforcement.
- Done: SDK/OpenAPI primitive property-type alignment guard now checks selected SDK-consumed schema property types, starting with `PolicyDeniedAuditMetadata` numeric quota hints (`incomingBytes`, `maxActiveSandboxes`, and `maxRetainedArtifactBytes`) so generated-client preparation catches number/string drift without changing audit behavior, quota semantics, or enforcement.
- Done: SDK/OpenAPI project quota-policy primitive type alignment now checks `ProjectQuotaPolicy` and `ProjectQuotaPolicyUpsert` numeric limit fields (`maxActiveSandboxes` and `maxRetainedArtifactBytes`) so generated-client preparation catches limit type drift without adding billing, capacity reservation, or new quota semantics.
- Done: SDK/OpenAPI artifact primitive type alignment now checks `Artifact`, `ArtifactCreate`, and `ArtifactContent` `sizeBytes` fields as numbers, keeping artifact clients generated-client ready without changing retained-content storage providers, download behavior, upload/capture limits, or quota enforcement.
- Done: SDK/OpenAPI runtime inventory primitive type alignment now checks `RuntimeResourceSummary.total` and `RuntimeResourceCount.count` as integers, keeping label-derived per-project runtime attribution generated-client ready without changing RBAC, quota, billing, metrics utilization, or capacity semantics.
- Done: SDK/OpenAPI runtime workload primitive type alignment now checks `RuntimeWorkloadSummary` Pod/readiness/restart rollup counters plus `RuntimeStorageSummary.count` as integers, keeping runtime inventory clients generated-client ready without adding metrics-server utilization, quota, billing, or capacity reservation semantics.
- Done: SDK/OpenAPI runtime observation primitive type alignment now checks `RuntimeResourceObservation` replica, Pod, readiness, and restart counters as integers, keeping per-resource runtime inventory details generated-client ready without adding metrics-server utilization, quota, billing, or capacity reservation semantics.
- Done: SDK/OpenAPI project usage count primitive type alignment now checks project-record sandbox/session/task/template/credential usage counters, resource request counts, quantity counters, and template request distribution counts as integers, keeping per-project usage clients generated-client ready without adding live metrics, billing, quota reservation, or capacity semantics.
- Done: SDK/OpenAPI project artifact usage primitive type alignment now checks project-record artifact kind counters plus referenced and retained byte counters as integers, keeping per-project artifact usage clients generated-client ready without changing retained-content storage, quota enforcement, billing, or capacity semantics.
- Done: SDK/OpenAPI timestamp format alignment guard now checks selected SDK-consumed timestamp fields keep OpenAPI `format: date-time`, including runtime inventory/orphan audit timestamps, project usage generation time, project/policy/member/credential/template/sandbox/session/task/artifact timestamps, artifact retained-content capture time, runtime events, and audit events, improving generated-client readiness without changing API behavior, storage, runtime behavior, or identity/audit semantics.
- Done: SDK/OpenAPI nullable-property alignment guard now checks selected PATCH clearing fields retain OpenAPI `nullable: true`, starting with `ProjectUpdate.defaultTemplateId` and `SandboxUpdate.runtimeRef`, preserving generated-client readiness for existing clear-field semantics without changing update behavior, storage, runtime projection, or lifecycle behavior.
- Done: SDK/OpenAPI command-array item type alignment guard now checks selected SDK-consumed command arrays such as `RuntimeTarget.commands`, `ExecutionTask.command`, `ExecutionTaskCreate.command`, and template `startupCommand` fields remain string arrays, keeping generated-client command contracts precise without changing task execution, template runtime projection, shell parsing, runtime behavior, or API behavior.
- Done: SDK/OpenAPI boolean primitive type alignment guard now checks selected SDK-consumed boundary and status booleans such as API authentication/RBAC capability flags, caller trust flags, project authorization decision flags, runtime orphan cleanup flags, preview availability, and task output truncation, keeping generated-client boolean contracts precise without changing authentication, RBAC enforcement, cleanup, preview, task execution, runtime behavior, or API behavior.
- Done: RBAC denial audit ergonomics now records optional matched project member hints for insufficient-role starter RBAC denials and renders them in the CLI policy-denied summary as a separate matched-member column, preserving best-effort audit semantics without expanding identity trust, route enforcement, or database schema.
- Done: SDK/OpenAPI UUID format alignment guard now checks selected SDK-consumed product record ID fields keep OpenAPI `format: uuid`, including project/template/sandbox/session/task/artifact, policy/member/credential, boundary, runtime inventory owner/orphan, audit event, and `policy.denied` metadata IDs, improving generated-client readiness without changing API behavior, persistence, runtime behavior, RBAC enforcement, audit identity, quota, billing, or capacity semantics.
- Done: SDK/OpenAPI string-map alignment guard now checks selected SDK-consumed `Record<string,string>` fields retain `additionalProperties.type=string`, covering template env maps, boundary resource requests, runtime inventory labels, and request/limit maps without changing API behavior, runtime behavior, RBAC enforcement, audit identity, quota, billing, or capacity semantics.
...
121. Done: CLI audit summary project-name ergonomics add `audit-events --summary --resolve-project-names` and `--policy-denied-summary --resolve-project-names`, resolving known project IDs through `/v1/projects` only for human-readable output while preserving raw JSON, audit API responses, best-effort audit semantics, and non-identity project attribution.
122. Done: SDK/OpenAPI policy-denial operation enum alignment guards `PolicyDeniedAuditMetadata.operation` values used by the SDK `PolicyDeniedOperation` union, so generated-client preparation catches operation drift without changing audit behavior, identity trust, or failure-event coverage.
123. Done: SDK/OpenAPI policy-denial RBAC/caller enum alignment guards `PolicyDeniedAuditMetadata.authorizationAction`, `callerMode`, `callerPrincipalType`, `principalType`, and `role` values used by SDK starter RBAC metadata types, keeping generated-client preparation aligned without changing audit behavior, identity trust, or route enforcement.
124. Done: SDK/OpenAPI policy-denial policy/quota enum alignment guards `PolicyDeniedAuditMetadata.policyKind` (`launch`/`quota`) and `enforcement` (`disabled`/`enforced`) values used by SDK policy-denial metadata types, keeping generated-client preparation aligned without changing audit behavior, quota semantics, identity trust, or route enforcement.
125. Done: SDK/OpenAPI primitive property-type alignment guard now checks selected SDK-consumed schema property types, starting with `PolicyDeniedAuditMetadata` numeric quota hints (`incomingBytes`, `maxActiveSandboxes`, and `maxRetainedArtifactBytes`) so generated-client preparation catches number/string drift without changing audit behavior, quota semantics, or enforcement.
126. Done: SDK/OpenAPI project quota-policy primitive type alignment now checks `ProjectQuotaPolicy` and `ProjectQuotaPolicyUpsert` numeric limit fields (`maxActiveSandboxes` and `maxRetainedArtifactBytes`) so generated-client preparation catches limit type drift without adding billing, capacity reservation, or new quota semantics.
127. Done: SDK/OpenAPI artifact primitive type alignment now checks `Artifact`, `ArtifactCreate`, and `ArtifactContent` `sizeBytes` fields as numbers, keeping artifact clients generated-client ready without changing retained-content storage providers, download behavior, upload/capture limits, or quota enforcement.
128. Done: SDK/OpenAPI runtime inventory primitive type alignment now checks `RuntimeResourceSummary.total` and `RuntimeResourceCount.count` as integers, keeping label-derived per-project runtime attribution generated-client ready without changing RBAC, quota, billing, metrics utilization, or capacity semantics.
129. Done: SDK/OpenAPI runtime workload primitive type alignment now checks `RuntimeWorkloadSummary` Pod/readiness/restart rollup counters plus `RuntimeStorageSummary.count` as integers, keeping runtime inventory clients generated-client ready without adding metrics-server utilization, quota, billing, or capacity reservation semantics.
130. Done: SDK/OpenAPI runtime observation primitive type alignment now checks `RuntimeResourceObservation` replica, Pod, readiness, and restart counters as integers, keeping per-resource runtime inventory details generated-client ready without adding metrics-server utilization, quota, billing, or capacity reservation semantics.
131. Done: SDK/OpenAPI project usage count primitive type alignment now checks project-record sandbox/session/task/template/credential usage counters, resource request counts, quantity counters, and template request distribution counts as integers, keeping per-project usage clients generated-client ready without adding live metrics, billing, quota reservation, or capacity semantics.
132. Done: SDK/OpenAPI project artifact usage primitive type alignment now checks project-record artifact kind counters plus referenced and retained byte counters as integers, keeping per-project artifact usage clients generated-client ready without changing retained-content storage, quota enforcement, billing, or capacity semantics.
133. Done: CLI policy-denied audit ergonomics now adds authorization action, trusted-caller boundary, and requested member principal/role hint columns to `audit-events --policy-denied-summary` and project-scoped policy-denied summaries, preserving raw JSON defaults, audit API responses, best-effort audit semantics, and non-identity attribution boundaries.
134. Done: CLI project authorization summary ergonomics now adds a derived operator hint for allowed, preflight-only, untrusted-caller, missing-member, and insufficient-role states over the existing authorization preflight response, preserving default JSON output, API shape, starter RBAC boundaries, and non-login identity semantics.
135. Done: SDK/OpenAPI schema alignment guard now checks selected primitive array item types for SDK-consumed string arrays such as project policy allow lists, project credential usage, project usage notes, boundary evidence, and boundary access/cleanup lists, keeping generated-client preparation tighter without changing API behavior, credential semantics, RBAC, quota, or runtime behavior.
136. Done: SDK/OpenAPI schema alignment guard now checks selected array item enum values for RBAC/authz arrays such as `ProjectRBACInfo.enforcedActions`, project authorization `requiredRoles`, and project authorization `availableActions`, tightening generated-client preparation without changing authentication, route enforcement, RBAC scope, or API behavior.
137. Done: CLI project usage ergonomics now supports `projects usage <project-id> --summary --runtime-attribution`, appending a second read-only runtime inventory lookup for the same project ID so operators can compare product-record usage with label-derived runtime resource, Pod, request/limit, restart, and PVC attribution without changing the usage API, quota semantics, RBAC, billing, metrics utilization, or runtime behavior.
138. Done: CLI policy-denied audit ergonomics now adds policy kind/enforcement plus quota limit and byte hint columns to global and project-scoped `--policy-denied-summary`, helping operators distinguish launch-policy, active-sandbox quota, retained-byte quota, and RBAC denials from returned best-effort metadata without changing audit APIs, identity trust, quota behavior, billing semantics, or route enforcement.
139. Done: SDK/OpenAPI schema alignment guard now checks selected SDK-consumed timestamp properties retain `format: date-time`, covering runtime inventory/orphan audit, project usage, project/policy/member/credential/template/sandbox/session/task/artifact, artifact retained-content, runtime event, and audit event timestamps without changing API behavior, runtime behavior, persistence, identity, audit, quota, or billing semantics.
140. Done: SDK/OpenAPI schema alignment guard now checks selected nullable PATCH clearing properties, currently `ProjectUpdate.defaultTemplateId` and `SandboxUpdate.runtimeRef`, keeping SDK null-clear semantics aligned with OpenAPI without changing API behavior, update persistence, runtime projection, lifecycle cleanup, RBAC, audit, quota, or billing semantics.
141. Done: SDK/OpenAPI schema alignment guard now checks selected SDK-consumed command arrays such as `RuntimeTarget.commands`, `ExecutionTask.command`, `ExecutionTaskCreate.command`, and template `startupCommand` fields remain string arrays, keeping generated-client command contracts precise without changing task execution, template runtime projection, shell parsing, runtime behavior, or API behavior.
142. Done: SDK/OpenAPI schema alignment guard now checks selected SDK-consumed boundary and status booleans such as API authentication/RBAC capability flags, caller trust flags, project authorization decision flags, runtime orphan cleanup flags, preview availability, and task output truncation, keeping generated-client boolean contracts precise without changing authentication, RBAC enforcement, cleanup, preview, task execution, runtime behavior, or API behavior.
143. Done: CLI project usage runtime-attribution ergonomics now supports `projects usage <project-id> --summary --runtime-attribution --resolve-project-names`, resolving known project IDs through a read-only `/v1/projects` lookup for the appended label-derived runtime attribution section while preserving default JSON output, usage API shape, runtime inventory API shape, and non-metrics/non-quota/non-RBAC semantics.
144. Done: RBAC denial audit ergonomics now records optional matched project member hints for insufficient-role starter RBAC denials and renders them in the CLI policy-denied summary as a separate matched-member column, preserving best-effort audit semantics without expanding identity trust, route enforcement, login/invite flows, or database schema.
145. Done: SDK/OpenAPI schema alignment guard now checks selected SDK-consumed product record ID properties retain `format: uuid`, covering project/template/sandbox/session/task/artifact records, policy/member/credential records, boundary and runtime inventory attribution, audit event IDs, and policy-denial metadata IDs without changing API behavior, persistence, runtime behavior, RBAC enforcement, audit identity, quota, billing, or capacity semantics.
146. Done: SDK/OpenAPI schema alignment guard now checks selected SDK-consumed string map fields retain `additionalProperties.type=string`, covering template env maps, boundary resource requests, runtime inventory labels, and request/limit maps while keeping generated-client preparation tighter without changing API behavior, runtime behavior, RBAC, audit, quota, billing, or capacity semantics.
147. Next: deeper generated-client/schema alignment, broader CLI ergonomics, or the next narrow RBAC/audit ergonomics slice that does not add login, invite flows, an agent brain, CI/CD platform, or deployment release manager primitives.
