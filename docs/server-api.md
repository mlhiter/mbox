...
- `audit-events` for recent product audit events. Add `--summary` for a compact action/resource/actor/source/request-id rollup over the returned rows, or `--policy-denied-summary` for a policy-denial operation/reason/request-id rollup.
