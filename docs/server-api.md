...
- `audit-events` for recent product audit events. Add `--summary` for a compact action/resource/project/actor/source/request-id rollup over the returned rows, or `--policy-denied-summary` for a policy-denial operation/reason/project/authz-action/caller/member/resource/request-id rollup; combine either summary mode with `--resolve-project-names` for a read-only project-name lookup in the human-readable table.
