...
Set `MBOX_TRUSTED_PRINCIPAL_HEADERS_ENABLED=true` only behind a trusted reverse proxy or in a local smoke environment. The server then reads `MBOX_TRUSTED_PRINCIPAL_HEADER` and `MBOX_TRUSTED_PRINCIPAL_TYPE_HEADER`, defaulting to `X-Mbox-Principal` and `X-Mbox-Principal-Type`. Accepted principal types are `user`, `service_account`, and `automation`. When both headers are present and valid, caller returns `mode: "trusted_header"` and `rbacTrusted: true`; project authorization preflight can match the caller against project member records. This does not perform login, user sessions, audit-label trust, or secret access. Set `MBOX_PROJECT_RBAC_ENFORCEMENT_ENABLED=true` only with trusted principal headers enabled; the current route-level guard covers `sandbox.launch`, `runtime.operate`, `artifact.write`, `policy.manage`, `credential.manage`, and `member.manage`. It denies sandbox creation, template validation launches, active runtime target/log/event/preview/terminal/session/task/workspace-content paths, artifact reference creation, workspace artifact capture, and client artifact-content upload unless the trusted caller matches an `owner` or `operator` project member. It denies project launch/quota policy updates, project credential-reference creation/deletion, and project member creation/deletion unless the trusted caller matches an `owner` project member. When both switches are enabled, project creation seeds the trusted caller as an owner member for that new project; ordinary product-record reads, including credential-reference and member list/get routes, remain outside this starter gate.
  ...
  "items": [
    {
      ...
      "observation": {
        ...
        "podCount": 1,
        "runningPodCount": 1,
        "podName": "resolved-sandbox-abcde",
        "podPhase": "Running",
        "containersReady": 1,
        "containersTotal": 1,
        "restartCount": 0,
...
The CLI keeps `mbox runtime orphans` as raw JSON by default for scripts. Operators can add `--summary-table` to render the same response as a compact read-only audit table with adapter, checked-at time, resource/orphan counts, reason counts, resource identity, project ID, sandbox status, message, and evidence lines. Add `--resolve-project-names` only with `--summary-table` when the CLI should make an additional read-only `/v1/projects` request and render known project IDs as `name (short-id)`. This does not add cleanup behavior or change the runtime orphan API response; it is only a human-readable view over `GET /v1/runtime/orphans`.
...
Project member records are unique by `(projectId, principalType, principal)` and create/delete operations write best-effort audit events as `project.member.created` and `project.member.deleted`. This is a project-scoped role registry for RBAC groundwork. The CLI keeps `projects members <project-id>` as JSON by default and adds `--summary` for role/principal-type counts plus member rows over the same read-only response. The current shared-token API does not authorize routes from these records, does not expose a trusted login session, and does not treat client-supplied audit actor/source labels as identity. When project RBAC enforcement and trusted principal headers are enabled, project creation seeds the trusted caller as an `owner` member for that new project, and subsequent member create/delete routes require an owner project member through `member.manage`.
...
The response includes `allowed`, `enforced`, `evaluation`, `requiredRoles`, `caller`, optional `matchedMember`, `memberCount`, `availableActions`, and explanatory `notes`. The CLI `projects authorization --summary` renders those fields with an operator hint for allowed, preflight-only, untrusted-caller, missing-member, or insufficient-role states, but the hint is derived client-side and is not a new API field. With the current anonymous/shared-token caller model, `enforced` is `false`, `allowed` is `false`, and `evaluation` is `not_enforceable`; this route does not deny product routes or promote member rows/audit labels into trusted identity unless the explicit project RBAC switch and trusted principal provider are both enabled for one of the starter enforced actions. Current route-level enforcement covers `sandbox.launch`, `runtime.operate`, `artifact.write`, `policy.manage`, `credential.manage`, and `member.manage`; the credential-management action gates only project credential-reference create/delete routes, not credential list/get reads, and the member-management action gates only member create/delete routes, not member list/get reads.
...
Project credential records are references only. mbox stores the Secret name/key and metadata, but not secret values, and the current runtime adapter does not mount these credentials into sandbox Pods. The CLI keeps `projects credentials <project-id>` as JSON by default and adds `--summary` for type, usage, Secret-reference counts, and credential-reference rows over the same read-only response. When project RBAC enforcement and trusted principal headers are explicitly enabled, credential-reference creation and deletion require an `owner` project member through the `credential.manage` action. Credential-reference list/get routes remain ordinary read visibility and do not expose secret values.

`GET /v1/projects/{projectID}/usage` is a read-only operational summary over mbox product records. It reports sandbox status counts, cleanup-pending soft-deleted sandboxes, runtime session counts, execution task status counts, artifact counts and retained bytes, visible template resource-request strings, active/running sandbox declared resource-request totals, and credential-reference counts. Sandbox request totals are derived by joining active sandbox product records to their saved templates and summing parseable Kubernetes quantity strings for CPU, memory, and storage; missing or invalid request strings are counted but do not make the usage route fail. The OpenAPI contract publishes this shape through `ProjectUsage`, `ProjectSandboxUsage`, `SandboxResourceRequestUsage`, and `ResourceQuantityUsage`. The CLI keeps `projects usage <project-id>` as JSON by default and adds `--summary` for a human-readable product-record rollup over the same response. Add `--runtime-attribution` only with `--summary` when an operator wants the CLI to make an additional read-only `GET /v1/runtime/resources?projectId=<project-id>` request and append label-derived runtime resource, Pod, request/limit, restart, and PVC state. Add `--resolve-project-names` with both flags when the CLI should make another read-only `GET /v1/projects` request and render known runtime attribution project IDs as `name (short-id)`. That appended section is a CLI composition over the runtime inventory, not a new usage API field. It does not read metrics-server utilization, and it is not billing, quota reservation, RBAC, or live cluster capacity. Project quota policies use the product-record aggregation for the currently implemented sandbox-count and retained-byte enforcement points.

`GET /v1/audit-events` and `GET /v1/projects/{projectID}/audit-events` list recent product audit events recorded after successful API write operations and selected policy/quota denials. Query filters are `projectId` for the global route, `action`, `resourceType`, `resourceId`, `actor`, `source`, `requestId`, `operation`, `reason`, `since`, `until`, and `limit` from 1 to 200. `since` and `until` are inclusive RFC3339 timestamps applied to `createdAt`; the server returns `400` if either timestamp is invalid or `since` is after `until`. `requestId` filters against `metadata.requestId`, which is present only on events written through an HTTP request carrying or receiving an mbox request ID. `operation` filters against `metadata.operation`, which is currently useful for typed `policy.denied` events such as `sandbox.launch`, `template.validation`, `project.policy.update`, `project.quota_policy.update`, `project.credential.create`, `project.credential.delete`, `project.member.create`, `project.member.delete`, `runtime.logs`, `runtime.session.create`, `execution.task.create`, `artifact.write`, `artifact.content.workspace.read`, `artifact.content.capture`, and `artifact.content.upload`; events without that metadata key do not match an operation filter. `reason` filters against `metadata.reason`, mainly for narrowing typed policy-denial feeds. Events include `action`, `resourceType`, optional project/resource IDs, resource name, actor, source, metadata, and creation time.

Clients can send `X-Mbox-Request-ID` to correlate an API response with server logs and any audit event written during that request. When a best-effort audit event is written, the same value is recorded in `metadata.requestId`; audit feeds can then be narrowed with `?requestId=...`. CLI and SDK clients expose request ID headers through `--request-id`, `MBOX_REQUEST_ID`, and SDK `requestId`, and expose audit-feed filtering through `--filter-request-id`, `--operation`, `--reason`, and SDK audit list `requestId` / `operation` / `reason`. CLI audit summary modes include returned project IDs and request IDs in their grouped tables when present, and policy-denial summaries additionally render metadata-derived policy kind/enforcement, quota limit and byte hints, authorization action, caller boundary, matched project member hints for insufficient-role RBAC denials, and requested member principal/role hints when present, so operators can jump from a noisy global feed back to specific project-scoped and request-correlated rows without printing raw metadata JSON. Add `--resolve-project-names` to those CLI summary modes when the CLI should make an additional read-only `/v1/projects` request and render known project IDs as `name (short-id)`; raw JSON output and audit API responses stay unchanged. Request IDs, audit project IDs, caller labels, matched/requested member hints, and quota hints improve traceability; they are not authentication, authorization, idempotency keys, quota enforcement changes, billing data, or a guarantee that every request writes an audit event.
...
This is a best-effort product-record audit starter: it currently records successful mbox API mutations such as project/template/sandbox writes, launch policy changes, quota policy changes, credential-reference and member changes, validation decisions, runtime session lifecycle, task creation/cancel requests, artifact creation/content retention, and gated runtime orphan cleanup. It also records `policy.denied` events for project launch policy denials, project RBAC denials, active sandbox quota denials, and retained artifact byte quota denials. The OpenAPI contract publishes `AuditEventAction` and `PolicyDeniedAuditMetadata`; for `policy.denied`, metadata always includes `operation` and `reason` and may include `requestId` for request/log correlation, plus typed policy/quota/member hints such as `policyKind` (`launch` or `quota`), `enforcement` (`disabled` or `enforced`), `maxActiveSandboxes`, `maxRetainedArtifactBytes`, requested member `principalType`/`principal`/`role`, and matched caller member `matchedMemberId`/`matchedMemberPrincipalType`/`matchedMemberPrincipal`/`matchedMemberRole` when present. Current RBAC denial `operation` values include sandbox launch, template validation launch, project launch/quota policy updates, project credential-reference create/delete, project member create/delete, active runtime operations, execution task operations, artifact write, workspace artifact read, capture, and upload paths. It is not yet a strong transactional audit log, auth identity model, or general failure-event stream.
...
The CLI keeps `templates boundary <template-id>` and `sandboxes boundary <sandbox-id>` as JSON by default and adds `--summary` for a human-readable view over the same response. The summary output is for operator triage only; it does not grant secret-value access, broaden RBAC, mount credentials, add custom NetworkPolicy projection, or read live utilization metrics.
...
The SDK exports `SDK_ROUTE_CONTRACT`, `SDK_SCHEMA_CONTRACT`, `checkOpenAPIAlignment`, `assertOpenAPIAlignment`, and `fetchAndAssertOpenAPIAlignment` as a starter route-alignment guard. The guard verifies SDK route-backed helpers against the published OpenAPI path, method, SDK-used query parameter set, route auth metadata, focused request bodies, and focused response shapes. It also checks reverse route coverage: ordinary published OpenAPI operations must have an SDK route contract entry. The intentional non-helper exceptions are the terminal WebSocket upgrade route and preview proxy pass-through route, which are not ordinary JSON SDK helpers. Auth checks cover the bearer security scheme, explicit public operations, private bearer operations, and `401` responses. Request checks cover JSON schema refs and binary upload media types. Response checks cover direct schema refs, list wrappers with required `items` arrays, list item refs, NDJSON task-event streams, binary responses, and no-content delete routes. It then checks a focused set of SDK-consumed schema required fields, properties, selected nullable properties, selected primitive property types, selected string property formats, selected string-map value types for SDK-consumed `Record<string,string>` fields, selected enum values, selected property/array item `$ref` contracts, selected primitive array item types, and selected array item enum values, including base health/error schemas, project schemas, project launch/quota policy schemas, credential-reference schemas, API info capability and compatibility schemas, caller/auth and project-authorization schemas, template-library and template-validation schemas, boundary summary/check schemas, sandbox/preview schemas, runtime inventory schemas, runtime target storage arrays, runtime log schemas, runtime session schemas, execution task schemas, artifact schemas, runtime orphan audit/cleanup schemas, and audit action/metadata refs plus the typed policy/quota fields and operation/policyKind/enforcement/RBAC/caller enum values on `PolicyDeniedAuditMetadata`; it can also flag selected forbidden properties such as immutable project, template, and sandbox update fields. It is not yet a generated client or full request/response schema validator. Usage, policy, quota, credential-reference, template, boundary, sandbox, preview, runtime logs, runtime session, execution task, artifact, auth, authorization, and audit contracts are no longer entirely loose objects: the SDK and OpenAPI both expose `/healthz` status, shared `Error.error`, list response `items` arrays, project response/create/update shapes, project update `defaultTemplateId` nullable clearing semantics, project policy/quota policy enforcement enums and numeric quota limit types, credential reference shapes, `secretRef` refs, string maps for template env, boundary resource requests, runtime labels, and request/limit maps, string-array item types for policy allow lists, credential usage, usage notes, and boundary evidence/access lists, and type enums without secret values, project member principal/role enums, project usage bucket/request/quantity count types, project usage artifact count/byte types, project usage bucket/request/quantity property refs, date-time formats for SDK-consumed resource, audit, session, task, sandbox, artifact, and policy timestamp fields, UUID formats for selected SDK-consumed product record ID fields, selected boolean boundary/status flags, SDK-consumed command/startup command arrays as string arrays, `APIInfo` runtime/artifact/trusted-header/RBAC/compatibility sub-schema refs, caller/auth boundary shapes, project authorization decision caller/member refs plus action/mode enum values and required-role/available-action array item enum values, template create/update shapes, template port and secret-ref array item refs, template-validation run refs, and validation decision enum values, structured `BoundarySummary` / `BoundaryCheck` / `BoundaryCredentialRef` fields, refs, and enum values, sandbox response/create/update runtimeRef and port item refs plus `runtimeRef` nullable clearing semantics and status enums, runtime resource owner kind enums, runtime inventory summary/workload/observation/storage count types, runtime reference and preview port shapes, runtime inventory summary/workload/resource property refs, runtime target `storage` item refs, preview-port target and item refs, runtime inventory workload nested-array item refs, runtime log `LogResult` target refs, runtime session response/create type, status, and runtimeRef contracts, execution task response/create/watch-event status/event/stream enums and runtimeRef contracts, artifact response/create kind enums, artifact and retained-content size types, retained-content metadata refs, and retained-content storage-provider enums, `$ref`-backed runtime orphan reason/status enums, runtime orphan/cleanup resource refs, the gated orphan-cleanup confirmation literal, project usage request-total types, known audit action and metadata refs, known audit action string types, the current `PolicyDeniedAuditMetadata` shape including numeric quota hint types, operation, policy kind, enforcement, authorization action, caller mode/principal type, principal type, and role values, and `isPolicyDeniedAuditEvent()` so clients can safely render selected denial events without treating all audit metadata as stable. `createMboxClientFromEnv()` mirrors the CLI environment convention for `MBOX_API_URL`, `MBOX_TOKEN`/`MBOX_API_TOKEN`, `MBOX_REQUEST_ID`, and audit labels, but it does not read CLI context files.
...
- `audit-events` for recent product audit events. Add `--summary` for a compact action/resource/project/actor/source/request-id rollup over the returned rows, or `--policy-denied-summary` for a policy-denial operation/reason/project/policy/quota/authz-action/caller/member/resource/request-id rollup; combine either summary mode with `--resolve-project-names` for a read-only project-name lookup in the human-readable table.
- `runtime resources` for the read-only managed runtime resource inventory. Add `--summary` to print only the filtered JSON `summary` object, or `--summary-table` for a human-readable total/grouping/workload summary; use `--project-id` to filter by runtime owner project label, and `--resolve-project-names` with `--summary-table` when an operator wants known project IDs rendered as `name (short-id)`.
- `runtime orphans` for the read-only runtime orphan audit. Use `--project-id` to inspect drift for one runtime owner project label, add `--summary-table` when an operator wants reason counts and actionable resource rows instead of raw JSON, and combine it with `--resolve-project-names` for a read-only project-name lookup in that human-readable table.
- `projects`: list, create, get, usage, authorization, audit-events, policy, set-policy, quota-policy, set-quota-policy, members, add-member, credentials, add-credential, delete. Add `projects usage <project-id> --summary` when an operator wants product-record counts and declared request totals in compact text instead of raw JSON; add `--runtime-attribution` with that summary when the operator also wants the CLI-composed, label-derived runtime inventory section for the same project, and add `--resolve-project-names` when that runtime attribution should render known project IDs as `name (short-id)` through a read-only project-list lookup. Add `projects audit-events <project-id> --summary` or `--policy-denied-summary` when an operator wants a read-only audit rollup over existing feed filters; add `projects policy <project-id> --summary` or `projects quota-policy <project-id> --summary` when an operator wants launch-policy or quota-policy boundaries rendered as text while preserving JSON defaults; add `projects members <project-id> --summary` when an operator wants starter RBAC role/principal-type counts over the member registry; add `projects credentials <project-id> --summary` when an operator wants credential-reference type/usage/Secret-reference counts without exposing secret values.
- `members`: get, delete.
- `credentials`: get, delete.
- `templates`: list, create, get, boundary, validate, validate-run, decide-validation, delete. Add `templates boundary <template-id> --project-id <project-id> --summary` when an operator wants the read-only runtime safety checks rendered as text. `templates validate-run <template-id> --project-id <project-id>` accepts repeated `--arg`, comma-split `--command`, JSON-array `--command-json`, or positional command arguments after `--`; it is a CLI-only composition over validation-run creation, sandbox wait, execution task creation/waiting, and validation decision, and does not add a server route or CI workflow model.
- `sandboxes`: list, create, get, boundary, start, stop, wait, delete. Add `sandboxes boundary <sandbox-id> --summary` when an operator wants the read-only resolved sandbox boundary rendered as text. `sandboxes wait <sandbox-id> --status running --require-runtime-ref` prints final sandbox JSON when ready, and exits nonzero after printing the final JSON if the sandbox reaches `failed` or `deleted` first.
- `sessions`: list, create, get, end.
- `tasks`: list, create, run, get, wait, cancel, watch, artifacts. The task-group usage output lists the same subcommands as the top-level help. `tasks create` accepts repeated `--arg`, comma-split `--command`, or JSON-array `--command-json` command input. `tasks run <sandbox-id>` accepts repeated `--arg`, comma-split `--command`, JSON-array `--command-json`, or positional command arguments after `--`; it creates an execution task and waits for its terminal task JSON. `--timeout` is the task execution timeout, while `--wait-timeout` is the client-side polling limit. `tasks wait --require-success` and `tasks run --require-success` keep stdout as final task JSON and return a nonzero exit when the terminal status is not `succeeded`. `tasks artifacts <task-id>` lists artifact references linked to one task.
