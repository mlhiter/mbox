...
- `audit-events` for recent product audit events. Add `--summary` for a compact action/resource/project/actor/source/request-id rollup over the returned rows, or `--policy-denied-summary` for a policy-denial operation/reason/project/request-id rollup.
