...
- read-only Runtime inventory view backed by `/v1/runtime/resources`, with live kind, namespace, owner, Pod phase/readiness, resource request, storage, label, checked-at visibility, and clickable label-derived project attribution for mbox-managed Kubernetes resources
...
- The Runtime rail item opens `#runtime` and shows read-only managed runtime resources, clickable project-label attribution filtering, Pod readiness, summed resource requests, and PVC state when the runtime auditor is configured.
