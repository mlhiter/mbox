...
- `ProjectUsage`: read-only product-record summary for project sandboxes, sessions, tasks, artifacts, visible templates, and credential references. It is not live Kubernetes metrics and does not enforce quota.
...
10. Soft-deletes sandboxes after template `lifecyclePolicy.ttlSeconds` expires.
11. Deletes the runtime claim when the sandbox is soft-deleted.
12. Guards project deletion until sandbox records are deleted and runtime references are cleared, so cleanup state is not lost through a cascading project delete.
13. Exposes runtime orphan audit that compares mbox-managed `SandboxClaim` and `SandboxTemplate` labels with product records.
14. Allows explicitly confirmed, one-resource orphan cleanup only after the server re-runs the audit and confirms the resource is still orphaned for the requested reason.

Project usage summaries are generated by the API server from Postgres product records. They make current operational shape visible before full quota enforcement or metrics integration exists: sandbox status counts, cleanup-pending rows, session/task history, artifact retained bytes, template resource-request strings, and credential-reference counts. They should stay factual and must not be presented as live cluster usage.
...
Runtime orphan audit treats Postgres product rows as the source of truth and Kubernetes labels as evidence. It can identify managed claims whose sandbox record is gone, soft-deleted claims that are waiting for reconciler cleanup, claims whose `runtimeRef` no longer matches the product record, and generated templates whose product template record is gone. Cleanup is not automatic: the API deletes only one currently reported orphan resource at a time, requires the expected reason and confirmation string, and should not replace the normal reconciler path for sandboxes that still have product records.
...
Artifacts are product metadata records for outputs produced by sandboxes or tasks. They store kind, name, URI, optional task linkage, content type, size, and metadata. The first durability path can capture small `workspace://` file bytes with size, sha256, and storage-provider metadata. The default provider keeps bytes in Postgres; the filesystem provider writes bytes under a configured local artifact directory; the S3-compatible provider writes retained bytes to an explicitly configured bucket. Postgres remains the metadata index in every mode. External URLs and object-store references that are not retained through mbox remain metadata-only. This gives external agents, SDK clients, CI systems, and humans a stable place to register output references such as workspace paths, HTTP URLs, reports, screenshots, images, and logs.

## Platform Primitives

The implemented slice covers projects, templates, sandboxes, runtime session records, sandbox-backed execution tasks, artifact references, retained workspace-file artifacts, launch policy, and project credential references. The next product layer should stay below agent and CI semantics:
...
The Runtime Workspace has tabs for Terminal, Sessions, Boundary, Storage, Preview, Tasks, Artifacts, Logs, and Events. Terminal is intentionally treated as a primary workspace surface, not as right-sidebar metadata.
