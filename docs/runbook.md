...
Use `--summary` on `audit-events` or `projects audit-events` when the raw feed is too noisy for first-pass triage by action. The CLI still reads the existing audit list route, preserves filters, and groups returned rows by action with count, latest timestamp, resource types, actors, sources, and request IDs found in returned metadata. Use `--policy-denied-summary` when the operator specifically wants policy-denial grouping by `metadata.operation` and `metadata.reason`; that mode forces `action=policy.denied` and includes request IDs when present. These are read-only operator summaries over best-effort product audit records; they do not add a new audit model, trusted identity source, or strong transactional audit guarantee.
