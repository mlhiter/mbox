...
go run ./cmd/mbox runtime resources --summary-table --resolve-project-names
...
go run ./cmd/mbox runtime orphans --project-id <project-id>
go run ./cmd/mbox runtime orphans --kind SandboxTemplate
go run ./cmd/mbox runtime orphans --summary-table --project-id <project-id>
go run ./cmd/mbox runtime orphans --summary-table --resolve-project-names --project-id <project-id>
curl -fsS http://127.0.0.1:18080/v1/runtime/orphans | jq
```

The runtime inventory reports the current auditor view plus `summary.total`, `summary.byKind`, `summary.byNamespace`, `summary.byOwner`, `summary.byProject`, and `summary.workload` for quick operator triage. Use `runtime resources --summary` when a script needs the filtered JSON summary object instead of every managed resource row; use `runtime resources --summary-table` when a human operator wants the same summary rendered as totals, grouping sections, and workload lines. Add `--resolve-project-names` only with `--summary-table` when an operator wants the CLI to make an additional read-only `/v1/projects` request and print known project IDs as `name (short-id)` in runtime resource or orphan summaries. The Web Runtime view uses the same `summary.byProject` object for clickable project attribution chips, so selecting a project in the UI applies the existing `projectId` runtime-resource filter rather than a separate product usage or permission model. Owner and project entries are derived from existing `mbox.dev/project-id`, `mbox.dev/sandbox-id`, and `mbox.dev/template-id` labels; they are not live metrics, RBAC, quota, billing, or capacity accounting. The orphan audit reports `missing-sandbox-record`, `cleanup-pending`, `runtime-ref-mismatch`, `missing-template-record`, and `unlabeled-owner`. Use `--namespace` / `?namespace=`, `--project-id` / `?projectId=`, and `--kind` / `?kind=` when a shared test cluster has older mbox-managed resources from prior runs. Project filtering only matches resources with a runtime owner project label, normally `SandboxClaim` rows, and is not an RBAC or quota decision. `runtime orphans --summary-table` keeps the same read-only audit request but renders adapter, checked-at time, resource/orphan counts, reason counts, resource identity, project ID, status, message, and evidence lines for human triage; default `runtime orphans` output remains JSON for scripts.
...
go run ./cmd/mbox projects usage <project-id> --summary --runtime-attribution
go run ./cmd/mbox projects usage <project-id> --summary --runtime-attribution --resolve-project-names
...
go run ./cmd/mbox audit-events --project-id <project-id> --summary --resolve-project-names --since 2026-05-30T00:00:00Z --until 2026-05-30T01:00:00Z
...
go run ./cmd/mbox projects audit-events <project-id> --policy-denied-summary --resolve-project-names --operation sandbox.launch --since 2026-05-30T00:00:00Z --until 2026-05-30T01:00:00Z
go run ./cmd/mbox projects policy <project-id> --summary
go run ./cmd/mbox projects quota-policy <project-id>
go run ./cmd/mbox projects quota-policy <project-id> --summary
...
go run ./cmd/mbox auth caller --summary
...
`projects usage <project-id>` returns the read-only product-record usage JSON for one project. Add `--summary` when a human operator wants a compact text view of sandbox/session/task/artifact/template/credential counts plus active/running declared sandbox request totals. Add `--runtime-attribution` only with `--summary` when the operator also wants the CLI to make a second read-only `/v1/runtime/resources?projectId=<project-id>` request and append label-derived runtime resource, Pod, request/limit, restart, and PVC state. Add `--resolve-project-names` with both flags when the CLI should make a third read-only `/v1/projects` request and render known runtime attribution project IDs as `name (short-id)`. Product-record request totals come from saved templates joined to product records, while runtime attribution comes from current runtime owner labels; neither is metrics-server utilization, billing, quota reservation, RBAC, or live cluster capacity.

`projects policy <project-id>` and `projects quota-policy <project-id>` keep JSON as the default scriptable output. Add `--summary` when a human operator wants the existing project launch-policy or quota-policy response rendered as compact text. The launch-policy summary is still only the sandbox creation and template validation launch gate for image prefixes, sandbox ServiceAccounts, and declared template secret reference names. The quota-policy summary is still only the product-record guard for active sandbox count and retained artifact bytes. These summaries do not add full RBAC, credential mounting, billing, capacity reservation, or live Kubernetes metrics.

`projects authorization <project-id> --action sandbox.launch` is RBAC preflight. It returns the roles required for the action, the current caller boundary, member count, and action-level `enforced` state. Add `--summary` to render the same response as a compact operator decision view while keeping the default JSON output for scripts; the summary includes an operator hint derived from the response to distinguish allowed, preflight-only, untrusted-caller, missing-member, and insufficient-role states. When trusted principal headers are explicitly enabled, the same route can return allowed or denied decisions by matching the trusted caller to project member records. With `MBOX_PROJECT_RBAC_ENFORCEMENT_ENABLED=true`, `sandbox.launch` is route-enforced for `POST /v1/sandboxes` and template validation launches, `runtime.operate` is route-enforced for active runtime target/log/event/preview/terminal/session/task/workspace-content paths, `artifact.write` is route-enforced for artifact reference creation, workspace artifact capture, and client artifact-content upload, `policy.manage` is route-enforced for project launch/quota policy updates, `credential.manage` is route-enforced for project credential-reference create/delete routes, and `member.manage` is route-enforced for member create/delete routes. Other actions remain preflight-only.

`auth caller` is a read-only caller boundary check. It reports anonymous local mode when no token gate is configured, shared-token mode after a matching bearer token is accepted, or trusted-header mode when a trusted reverse-proxy/test principal provider is explicitly enabled. Add `--summary`, or use `caller --summary`, when an operator wants compact text instead of JSON. The response includes `rbacTrusted` and `projectRolesEnforced`; trusted headers can make `rbacTrusted=true`, while `projectRolesEnforced=true` only means the starter `sandbox.launch`, `runtime.operate`, `artifact.write`, `policy.manage`, `credential.manage`, and `member.manage` gates are enabled.
...
Use `--summary` on `audit-events` or `projects audit-events` when the raw feed is too noisy for first-pass triage by action. The CLI still reads the existing audit list route, preserves filters, and groups returned rows by action with count, latest timestamp, resource types, project IDs, actors, sources, and request IDs found in returned metadata. Use `--policy-denied-summary` when the operator specifically wants policy-denial grouping by `metadata.operation` and `metadata.reason`; that mode forces `action=policy.denied` and includes project IDs, policy kind/enforcement, quota limit and byte hints, authorization action, trusted-caller boundary, matched project member hints for insufficient-role RBAC denials, requested member principal/role hints, actors, sources, resources, and request IDs when present. Add `--resolve-project-names` only with one of these summary modes when the CLI should make an additional read-only `/v1/projects` request and render known project IDs as `name (short-id)` in the summary table. These are read-only operator summaries over best-effort product audit records; they do not add a new audit model, trusted identity source, project permission decision, quota engine, billing view, or strong transactional audit guarantee.
...
`check:openapi` builds the SDK, fetches `/v1/openapi.json` from the API base URL, and verifies every route-backed SDK helper in `SDK_ROUTE_CONTRACT` has a matching OpenAPI path, method, route auth metadata, focused request shape, and focused response shape. It sends `MBOX_TOKEN` or `MBOX_API_TOKEN` as a bearer token when either is set, and it can also accept a saved OpenAPI JSON file path. When project, policy, quota, credential-reference, template, sandbox, preview, runtime session, execution task, artifact, usage, audit, request correlation, auth, authorization, or boundary contracts change, also check that OpenAPI schemas such as `Project`, `ProjectCreate`, `ProjectUpdate`, `ProjectPolicy`, `ProjectPolicyUpsert`, `ProjectQuotaPolicy`, `ProjectQuotaPolicyUpsert`, `ProjectCredential`, `ProjectCredentialCreate`, `SecretRef`, `CallerInfo`, `ProjectAuthorizationDecision`, `EnvironmentTemplate`, `TemplateCreate`, `TemplateUpdate`, `TemplateValidationRunCreate`, `TemplateValidationRunDecision`, `TemplateValidationRun`, `TemplatePort`, `BoundarySummary`, `BoundaryCheck`, `BoundaryPort`, `BoundaryCredentialRef`, `RuntimeRef`, `SandboxPort`, `Sandbox`, `SandboxCreate`, `SandboxUpdate`, `PreviewPort`, `PreviewPortsResult`, `RuntimeResourceOwner`, `RuntimeSession`, `RuntimeSessionCreate`, `ExecutionTask`, `ExecutionTaskCreate`, `ExecutionTaskEvent`, `Artifact`, `ArtifactCreate`, `ArtifactContent`, `RuntimeOrphan`, `RuntimeOrphanCleanupRequest`, `RuntimeOrphanCleanupResult`, `ProjectUsage`, `ProjectSessionUsage`, `ProjectTaskUsage`, `ProjectArtifactUsage`, `ProjectTemplateUsage`, `ProjectCredentialUsage`, `SandboxResourceRequestUsage`, `ResourceUsageValue`, `AuditEventAction`, `PolicyDeniedAuditMetadata` including policy/quota denial hints, selected auth/authorization enum values, auth/authorization array item enum values, project policy/quota enforcement values, project member principal/role values, project credential type values, string-array item types, boundary kind/check status/credential type values, runtime resource owner kind values, runtime/task/artifact enum values, runtime orphan reason/status values, template validation decision values, the gated orphan-cleanup confirmation literal, and the `bearerAuth` security scheme still match the exported SDK contract.
