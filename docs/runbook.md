...
Use `--summary` on `audit-events` or `projects audit-events` when the raw feed is too noisy for first-pass triage by action. The CLI still reads the existing audit list route, preserves filters, and groups returned rows by action with count, latest timestamp, resource types, project IDs, actors, sources, and request IDs found in returned metadata. Use `--policy-denied-summary` when the operator specifically wants policy-denial grouping by `metadata.operation` and `metadata.reason`; that mode forces `action=policy.denied` and includes project IDs, authorization action, trusted-caller boundary, requested member principal/role hints, actors, sources, resources, and request IDs when present. Add `--resolve-project-names` only with one of these summary modes when the CLI should make an additional read-only `/v1/projects` request and render known project IDs as `name (short-id)` in the summary table. These are read-only operator summaries over best-effort product audit records; they do not add a new audit model, trusted identity source, project permission decision, or strong transactional audit guarantee.
